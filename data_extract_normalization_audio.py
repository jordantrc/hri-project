...
from object_naming_audio_cnn import SampleGenerator
...
for i, t in enumerate(tfrecords):
    ...
    aud.append(SampleGenerator.get_sample_from_tfrecord(t, 'aud_raw', aud_dtype))
    end = time.time()
...
with open('data/audio_raw.csv', 'wb') as csvfile:
    writer = csv.writer(csvfile)
    for r in aud:
        img_data = r[0].tolist()
        label_one_hot = r[1].tolist()
