...
def train_cnn(train, test, model_name, data_type):
    ...
    with tf.Session() as sess:
        ...
        plot_confusion_matrix(cm, CLASSES, "plots/" + model_name + ".png", title=plot_title)
...
def balanced_samples(data, n):
    ...
    while len(samples) < n:
        samples.append(random.choice(data[CLASSES[c]]))
        c += 1
...
def main():
    ...
    num_classes = len(CLASSES)
    data_name = "top_grs_raw"
    data_type = grs_resize_dtype
    data_file = data_dir + data_name + "_64x64.csv"

    data_dict = parse_csv_data(data_file)

    # create train and test sets
    num_train_samples = int(data_dict['length'] * TRAIN_SET_SIZE)
    num_test_samples = data_dict['length'] - num_train_samples
    train = balanced_samples(data_dict, num_train_samples)
    test = balanced_samples(data_dict, num_test_samples)

    if SHUFFLE:
        random.shuffle(train)
        random.shuffle(test)

    print("training with %s samples, testing with %s samples" % (num_train_samples, num_test_samples))

    train_cnn(train, test, data_name, data_type)
